...
/**
 * Created by Metehan on 5/22/2016.
 */
...
public class IPPacket {
    ...
    void setPayload(int headerLengths, byte[] payload) {
        ...
        if (MyVpnService.isTestSrcAddress()) {
            Log.d("IPPacket: ", "payload == " + Arrays.toString(payload));
            Log.d("IPPacket: ", "mPayload == " + Arrays.toString(mPayload));
        }
    }
    ...
    void calculateIpHeaderCheckSum() {
        ...
        if (MyVpnService.isTestSrcAddress()) {
            Log.d("IPPacket: ", "ipv4Header == " + Arrays.toString(ipv4Header));
        }
    ...
    void updateUdpCheckSum() {
        if (MyVpnService.isTestSrcAddress()) {
            Log.d("IPPacket: ", "mPayload.length == " + mPayload.length);
            Log.d("IPPacket: ", "ipv4PseudoHeader.length == " + (IPV4_PSEUDO_PRE_UDP_PART_LENGTH + UDP_HEADER_LENGTH + mPayload.length));
        }
        ...
        if (MyVpnService.isTestSrcAddress()) {
            Log.d("IPPacket: ", "ipv4PseudoHeader == " + Arrays.toString(ipv4PseudoHeader));
        }
